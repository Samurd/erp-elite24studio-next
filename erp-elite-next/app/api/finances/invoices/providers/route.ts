...
import { count, desc, eq, and, like, or, gte, lte, sql } from "drizzle-orm";
import { z } from "zod";
...
async function generateInvoiceCode() {
    ...
    if (!isUnique) {
        code = `INV-PRV-${dateStr}-${Date.now().toString().slice(-6)}`;
    }
...
export async function GET(req: NextRequest) {
    try {
        ...
        // 1. Get 'Proveedor' tag ID for contact relation type
        const providerTagResult = await db
            .select({ id: tags.id })
            .from(tags)
            .innerJoin(tagCategories, eq(tags.categoryId, tagCategories.id))
            .where(and(
                eq(tagCategories.slug, "tipo_relacion"),
                eq(tags.name, "Proveedor")
            ))
            .limit(1);

        const providerTag = providerTagResult[0];
        ...
        const queryConditions = [
            like(invoices.code, "INV-PRV-%"),
        ];

        if (providerTag) {
            queryConditions.push(eq(contacts.relationTypeId, providerTag.id));
        }
...
export async function POST(req: NextRequest) {
    try {
        ...
        console.error("Error creating provider invoice:", error);
        return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
    }
}
