...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        // Use select with manual joins instead of db.query
        ...
        if (!row) return NextResponse.json({ error: "Billing Account not found" }, { status: 404 });
        ...
        const invoice = {
            ...
            contactId: row.contactId, // For form default values
            statusId: row.statusId, // For form default values
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        // Check existence with select
        const currentResult = await db.select({ id: invoices.id, code: invoices.code }).from(invoices).where(eq(invoices.id, id)).limit(1);
        if (currentResult.length === 0) return NextResponse.json({ error: "Billing Account not found" }, { status: 404 });
